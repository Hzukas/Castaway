...
export default function Onboarding() {
  ...
  function goNext() {
    ...
    if (joinCode) { localStorage.removeItem('joinAfterAuth'); window.location.href = `/join/${joinCode}` }
  ...
  return (
    <main style={{minHeight:'100vh',background:'#0d1f2d',display:'flex',alignItems:'center',justifyContent:'center',fontFamily:'sans-serif',position:'relative',overflow:'hidden'}}>
      <svg style={{position:'absolute',inset:0,width:'100%',height:'100%'}} xmlns="http://www.w3.org/2000/svg">
        {[0,1,2,3,4,5,6,7,8].map(i=>(<line key={`h${i}`} x1="0" y1={`${i*12.5}%`} x2="100%" y2={`${i*12.5}%`} stroke="#378ADD" strokeWidth="0.5" strokeOpacity={i%2===0?0.1:0.05}/>))}
      ...
      <div style={{position:'relative',zIndex:1,background:'rgba(255,255,255,0.05)',border:'0.5px solid rgba(255,255,255,0.1)',borderRadius:'16px',padding:'2.5rem',width:'100%',maxWidth:'440px'}}>
