...
export default function Auth() {
  ...
  async function routeAfterAuth(user) {
    ...
    if (!profile || !profile.display_name) {
      ...
      if (joinCode) { localStorage.removeItem('joinAfterAuth'); dest = `/join/${joinCode}` }
      else dest = '/dashboard'
    }
  ...
  return (
    <main style={{minHeight:'100vh',background:'#0d1f2d',display:'flex',alignItems:'center',justifyContent:'center',fontFamily:'sans-serif'}}>
      <div style={{background:'rgba(255,255,255,0.05)',border:'0.5px solid rgba(255,255,255,0.1)',borderRadius:'16px',padding:'2.5rem',width:'100%',maxWidth:'400px'}}>
        ...
        {message && (
          <div style={{background:'rgba(255,209,102,0.1)', border:'0.5px solid rgba(255,209,102,0.3)', borderRadius:'8px', padding:'10px 14px', marginBottom:'16px', color:'#FFD166', fontSize:'13px'}}>{message}</div>
        )}

        <button onClick={handleSubmit} disabled={loading} style={{width:'100%', padding:'13px', borderRadius:'10px', background:'#FFD166', color:'#1a0e00', border:'none', fontSize:'15px', fontWeight:700, cursor:'pointer'}}>
